...
function compute_step(T2, T, Cp, lam, dt, _dx, _dy, sig_real, signals, qs,
                      threads, grid, b_width, nt, me)
...
@views function diffusion2D(; do_vis = false)
    ...
    # Time loop
    compute_step(T2, T, Cp, lam, dt, _dx, _dy, sig_real, signals, qs, threads,
                 grid, b_width, 12, me) # warmup

    # Profile.clear()

    wtime = compute_step(T2, T, Cp, lam, dt, _dx, _dy, sig_real, signals, qs,
                         threads, grid, b_width, nt, me)
    ...
    if do_vis
        ...
        if (me == 0)
            ...
            png("../output/Temp_hide_$(nprocs)_$(nx_g())_$(ny_g()).png")
        end
    end
