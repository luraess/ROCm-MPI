...
function compute_step(T2, T, Cp, lam, dt, _dx, _dy, _dz, sig_real, signals, qs,
...
@views function diffusion3D(; do_vis = false)
    ...
    # Time loop
    compute_step(T2, T, Cp, lam, dt, _dx, _dy, _dz, sig_real, signals, qs,
                 threads, grid, b_width, 12, me) # warmup

    wtime = compute_step(T2, T, Cp, lam, dt, _dx, _dy, _dz, sig_real, signals,
                         qs, threads, grid, b_width, nt, me)
