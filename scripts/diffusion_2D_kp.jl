...
macro d_xa(A, ix, iy)
    esc(:(($A[$ix + 1, $iy + 0] - $A[$ix, $iy + 0])))
end
macro d_ya(A, ix, iy)
    esc(:(($A[$ix + 0, $iy + 1] - $A[$ix + 0, $iy])))
end
macro d_xi(A, ix, iy)
    esc(:(($A[$ix + 1, $iy + 1] - $A[$ix, $iy + 1])))
end
macro d_yi(A, ix, iy)
    esc(:(($A[$ix + 1, $iy + 1] - $A[$ix + 1, $iy])))
end
macro inn(A, ix, iy)
    esc(:(($A[$ix + 1, $iy + 1])))
end
...
function Residual!(dTdt, qx, qy, Cp, _dx, _dy)
    ...
    if (ix <= size(dTdt, 1) && iy <= size(dTdt, 2))
        @all(dTdt, ix, iy) = 1.0 / @inn(Cp, ix, iy) *
                             -(@d_xa(qx, ix, iy) * _dx +
                               @d_ya(qy, ix, iy) * _dy)
    end
    return
...
@views function diffusion2D()
    ...
    # Initial conditions
    T = ROCArray([exp(-(x_g(ix, dx, T) + dx / 2 - lx / 2)^2 -
                      (y_g(iy, dy, T) + dy / 2 - ly / 2)^2)
                  for ix in 1:size(T, 1), iy in 1:size(T, 2)])
    # visu
    if (me == 0)
        gr()
        ENV["GKSwstype"] = "nul"
    ...
    for it in 1:nt
        wait(@roc groupsize=threads gridsize=grid Flux!(qx, qy, T, lam, _dx,
                                                        _dy))
        wait(@roc groupsize=threads gridsize=grid Residual!(dTdt, qx, qy, Cp,
                                                            _dx, _dy))
    ...
    gather!(T_nh, T_v)
    if (me == 0)
        heatmap(transpose(T_v))
        png("../output/Temp_kp_$(nprocs)_$(nx_g())_$(ny_g()).png")
    end
